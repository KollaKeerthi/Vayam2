...
function QuestionCard({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
    >
      <Card className={`relative overflow-hidden group transition-all duration-300 border-border/50 hover:border-primary/30 hover:shadow-md ${
        ...
        <CardHeader className="flex flex-row items-start gap-3 pb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground line-clamp-2 mb-3">{question.description}</p>
...
export default function AdminQuestionsDashboard() {
  ...
  const [questionToInvite, setQuestionToInvite] = useState<Question | null>(null)
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      const newEmails = formData.allowedEmails.filter(email => !oldEmails.includes(email))
      ...
      if (response.data.success) {
        ...
        if (newEmails.length > 0) {
          try {
            ...
            toast.success(`Question updated and invitations sent to ${newEmails.length} new SME(s)!`)
          } catch (emailError) {
            console.error("Error sending invitations:", emailError)
            toast.success("Question updated successfully, but some invitations failed to send")
          }
        } else {
  ...
  const openEditDialog = (question: Question) => {
    setFormData({
      ...
      allowedEmails: Array.isArray(question.allowedEmails) ? question.allowedEmails : [],
  ...
  const totalStats = questions.reduce(
    (acc, question) => ({
      ...
      activeQuestions: acc.activeQuestions + (question.isActive ? 1 : 0),
      privateQuestions: acc.privateQuestions + 1, // All questions are private now
      totalParticipants: acc.totalParticipants + (question.participantCount || 0),
    }),
    { totalQuestions: 0, activeQuestions: 0, privateQuestions: 0, totalParticipants: 0 }
  )
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground">
                  {selected ? "Manage this question" : "Manage all questions and their settings"}
                </p>
              </div>
              {!selected && (
                ...
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-2xl font-bold text-foreground">{totalStats.totalQuestions}</p>
                ...
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-2xl font-bold text-foreground">{totalStats.activeQuestions}</p>
                ...
                <Card className="border-border/50 shadow-sm">
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-2xl font-bold text-foreground">{totalStats.totalParticipants}</p>
                  ...
                  {filteredQuestions.length === 0 ? (
                    ...
                    >
                      ...
                      <p className="text-muted-foreground mb-4">
                        {searchQuery ? "Try adjusting your search terms" : "Get started by creating your first question"}
                      </p>
                      {!searchQuery && (
                    ...
                    filteredQuestions.map((question) => {
                      ...
                      const isSelected = selectedQuestion ? selectedQuestion.id === question.id : false
                      return (
                        <QuestionCard
                          key={question.id}
                          question={question}
                  ...
                  <div className="space-y-6">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selected.tags.map((tag) => (
                            <Badge key={tag} variant="outline" className="bg-muted/50 border-border/50">
                              {tag}
                            </Badge>
                          ))}
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <div className="text-center p-4 bg-muted/30 rounded-xl">
                        ...
                        <p className="text-2xl font-bold text-foreground">{selected.participantCount}</p>
                    ...
                    {selected.allowedEmails.length > 0 && (
                      <div className="mt-4 p-4 bg-muted/30 rounded-xl">
                        <h4 className="font-medium text-foreground mb-2">SME Access List:</h4>
                        <div className="flex flex-wrap gap-2">
                    ...
                    <div className="flex gap-2 pt-4">
                      <Button onClick={() => openEditDialog(selected)} className="flex items-center gap-2">
          ...
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            <DialogContent className="sm:max-w-[525px]">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <TagInput
                    ...
                    onChange={(emails) => setFormData({ ...formData, allowedEmails: emails })}
                    placeholder="Add email addresses..."
                  />
                  <p className="text-xs text-muted-foreground">
                    SMEs will receive email invitations to contribute to this question
                  </p>
                </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleCreate} disabled={!formData.title.trim() || dashboardLoading}>
          ...
          <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
            <DialogContent className="sm:max-w-[525px]">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  <Label htmlFor="edit-allowedEmails">SME Email Addresses</Label>
                  <TagInput
                    tags={formData.allowedEmails}
                    onChange={(emails) => setFormData({ ...formData, allowedEmails: emails })}
                    placeholder="Add email addresses..."
                  />
                  <p className="text-xs text-muted-foreground">
                    SMEs will receive email invitations to contribute to this question. New emails will be automatically invited.
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <Switch
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleEdit} disabled={!formData.title.trim() || dashboardLoading}>
                  <Save className="h-4 w-4 mr-2" />
                  Update Question
                </Button>
              </div>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="py-4">
                <p className="text-sm text-muted-foreground">
                  Are you sure you want to delete this question? This action cannot be undone and will remove all
                  associated data.
                </p>
                {questionToDelete && (
                  <div className="mt-4 p-3 bg-muted rounded-md">
                    <p className="font-medium text-sm">{questionToDelete.title}</p>
                    <p className="text-xs text-muted-foreground mt-1">{questionToDelete.description}</p>
                  </div>
                )}
              </div>
              <div className="flex justify-end gap-2">
                ...
                <Button variant="destructive" onClick={handleDelete} disabled={dashboardLoading}>
            ...
            <DialogContent className="sm:max-w-[525px]">
              ...
              <div className="py-4">
                <p className="text-sm text-muted-foreground mb-4">
                  Send email invitations to Subject Matter Experts for this question:
                </p>
                {questionToInvite && (
                  <div className="mb-4 p-3 bg-muted rounded-md">
                    <p className="font-medium text-sm">{questionToInvite.title}</p>
                  </div>
                )}
                <div className="grid gap-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    These users will receive email invitations to contribute to this question
                  </p>
                </div>
              </div>
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleInviteSME} disabled={inviteEmails.length === 0 || dashboardLoading}>
